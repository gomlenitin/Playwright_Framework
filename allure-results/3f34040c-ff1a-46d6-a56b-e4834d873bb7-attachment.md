...
- Name: tests\Projects\Project_4_TTA_Bank.copy.spec.ts >> Transfer money and verify the final balance in TTA bank application
- Location: tests\Projects\Project_4_TTA_Bank.copy.spec.ts:3:1
...
```ts
  ...
  17 |     //await page.locator('//button[normalize-space()="Transfer Funds"]').click();
  18 |     await page.getByText('Transfer Funds').click();
  19 |     // await page.locator('//input[@type="number"]').fill('10000.75');
  20 |     // await page.locator('//input[@type="text"]').fill('Rent-Test');
  21 |     //let tranferAmount:any = await page.locator('//input[@type="number"]').inputValue();
  22 |     // await page.locator('//button[text()="Continue"]').click();
  23 |     // await page.locator('//button[text()="Confirm Transfer"]').click();
  24 | 
  25 |         // Fill in the Transfer Funds Form to Transfer $5000 to the account number 1234567890
  26 |     await page.getByPlaceholder('0.00').fill('5000');
  27 |     await page.getByPlaceholder('e.g. Rent for October').fill('Transfering $5000 to account number 1234567890');
  28 |     let tranferAmount:any = await page.locator('//input[@type="number"]').inputValue();
  29 |     await page.getByRole('button', { name: 'Continue' }).click();
  30 | 
  31 |     // Review the Transfer Details and Click on Confirm Transfer Button
  32 |     await page.getByRole('button', { name: 'Confirm Transfer' }).click();
  33 | 
  34 |     // Navigate to Dashboard and Validate the Balance is reduced
  35 |     await page.getByText('Dashboard').click();
  36 |     //await page.locator('//button[normalize-space()="Dashboard"]').click();
  37 |     let finalBalance:any = initialBalance - tranferAmount;
  38 |     console.log("Final Balance:", finalBalance);
  39 |     await page.waitForTimeout(3000);
> 40 |     expect(((await page.locator("//p[text()='Total Balance']/following-sibling::h3").innerText()).replace(/[^0-9.]/g,''))).toBe(finalBalance);
     |                                                                                                                            ^ Error: expect(received).toBe(expected) // Object.is equality
  41 | 
  42 |    
  43 | 
